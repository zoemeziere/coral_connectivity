...
sigmaFrom_b <- function(D, b, b_high, b_low, dims){
  if(dims==1){
    sigma <- sqrt(1/(4*De*b)) 
    sigma_low <- sqrt(1/(4*D*b_low)) 
    sigma_high <- sqrt(1/(4*D*b_high)) 
  }
  if(dims==2){
    sigma <- sqrt(1/(pi*4*D*b)) 
    sigma_low <- sqrt(1/(pi*4*D*b_low)) 
    sigma_high <- sqrt(1/(pi*4*D*b_high)) 
  }
...
sigma_Spis_De <- sigmaFrom_b(De_Spis, b_Spis, b_low_Spis, b_high_Spis, dims_Spis)
sigma_Spis_Dc <- sigmaFrom_b(Dc_Spis, b_Spis, b_low_Spis, b_high_Spis, dims_Spis)
...
sigma_Pver_De <- sigmaFrom_b(De_Pver, b_Pver, b_low_Pver, b_high_Pver, dims_Pver)
sigma_Pver_Dc <- sigmaFrom_b(Dc_Pver, b_Pver, b_low_Pver, b_high_Pver, dims_Pver)
